...
# -------------------------------
# OPTIMIZED: Read batch data with MGET
# -------------------------------
def read_batch_data_optimized(src_url, src_token, keys):
    """
    OPTIMIZED VERSION: Uses MGET instead of individual DUMP commands
    Reduces operations by ~66% (1 MGET vs 3 commands per key)
    
    Note: This only works for STRING type keys. 
    If you have hashes/lists/sets, use the original DUMP method.
    """
    if not keys:
        return {}
    
    # Single MGET command for all keys in batch (1 operation instead of N*3)
    commands = [["MGET"] + keys]
    ...
    values = results[0]['result']
    
    # Build key-value pairs
    key_data = {}
    for key, value in zip(keys, values):
        if value is not None:  # Skip non-existent keys
            key_data[key] = {
                'type': 'string',  # MGET only works with strings
                'value': value,
                'ttl': -1  # We skip TTL to save operations
            }
...
# Read a single batch
# -------------------------------
def read_batch(config, keys, batch_id, total_batches, use_optimized=True):
    """Read one batch of keys"""
    try:
        if use_optimized:
            key_data = read_batch_data_optimized(config['src_url'], config['src_token'], keys)
        else:
            # Use original method if you need full data types
            key_data = read_batch_data_original(config['src_url'], config['src_token'], keys)
...
# -------------------------------
def read_batch_data_original(src_url, src_token, keys):
    """Original method with TYPE, DUMP, PTTL"""
    if not keys:
        return {}
...
# -------------------------------
def export_to_json(use_optimized=True):
    config = get_config()
    ...
    print("="*60)
    print(f"📥 EXPORTING REDIS DATABASE TO JSON")
    print(f"⚡ Mode: {'OPTIMIZED (MGET)' if use_optimized else 'FULL (TYPE+DUMP+PTTL)'}")
    print("="*60)
    ...
    if use_optimized:
        estimated_ops = len(all_keys) // BATCH_SIZE + len(all_keys) // BATCH_SIZE
        print(f"\n💡 Estimated operations: ~{estimated_ops:,} (SCAN + MGET)")
    else:
        estimated_ops = len(all_keys) * 3
        print(f"\n💡 Estimated operations: ~{estimated_ops:,} (TYPE + DUMP + PTTL)")
    
    # Step 2: Split into batches
    batches = [all_keys[i:i + BATCH_SIZE] for i in range(0, len(all_keys), BATCH_SIZE)]
    total_batches = len(batches)
    
    print(f"📦 Created {total_batches} batches of up to {BATCH_SIZE} keys each")
    ...
    with ThreadPoolExecutor(max_workers=THREADS) as executor:
        ...
        for i, batch in enumerate(batches, 1):
            future = executor.submit(read_batch, config, batch, i, total_batches, use_optimized)
    ...
    output = {
        "metadata": {
            ...
            "method": "optimized_mget" if use_optimized else "full_dump"
        },
        "keys": all_data
    }
...
if __name__ == "__main__":
    # Set to True for optimized mode (~2k ops for 96k keys)
    # Set to False for full backup mode (~288k ops for 96k keys)
